    ...
    public ResultDto<ResultUserLoginDto> Execute(string Username, string Password)
    {
        ...
        if (user == null)
            return new ResultDto<ResultUserLoginDto>
            {
                Data = new ResultUserLoginDto(),
                IsSuccess = false,
                Message = "کاربری با این ایمیل در سایت ثبت نام نکرده"
            };
        var passwordHasher = new PasswordHasher();
        var resultVerifyPassword = passwordHasher.VerifyPassword(user.Password, Password);
        ...
        foreach (var item in user.UserInRoles) roles.Add(item.Role.Name);
